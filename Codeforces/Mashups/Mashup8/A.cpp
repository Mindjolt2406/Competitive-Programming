...
int main()
{
  ...
  vector<int> v(m),cost(n),end(n);
  Matrix<int> B(m,1,0,MOD);
  Matrix<int> A(m,m,0,MOD);
  ...
  for(int i=0;i<n;i++) {cin>>cost[i];cost[i]%=m;}
  for(int i=0;i<n;i++) {cin>>end[i];end[i]%=m;}
...
1. Didn't see the memory, randomly made the array w which goes up to size 10^8
2. My matrix library code wasn't correct, hence the WA. 
