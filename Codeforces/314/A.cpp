...
int main()
{
  ...
  {
    ll min1 = INF;
    if(i>0) min1 = min(min1,v[i]-v[i-1]);
    if(i<n-1) min1 = min(min1,v[i+1]-v[i]);
    minx[i] = min1;
    maxx[i] = max(abs(v[i]-left),abs(v[i]-right));
  }

  for(int i=0;i<n;i++) cout<<minx[i]<<" "<<maxx[i]<<endl;
  return 0;
}
