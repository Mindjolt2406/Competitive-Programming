...
signed main()
{
	...
	for(int i=1;i<=h-1;i++)
	{
		good.push_back({(m*h*i-(a+eps))/(h-1),(m*h*i+(a+eps))/(h-1)});
	}
	tr(good);
	set<int> timestamps;//minutes in goodranges
	for(int i=0;i<=1+good.back().second;i++)
	{
		bool flag = 0;
		for(auto x: good){
    ...
    int startMid = a + 1, endMid = midIntervalSize + a - 1;
    tr(startMid / (h - 1), endMid / (h - 1));
}