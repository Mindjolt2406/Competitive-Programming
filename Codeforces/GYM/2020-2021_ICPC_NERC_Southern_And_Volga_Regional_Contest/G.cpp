...
int main()
{
  __;
	...
	seen[n] = seen[n-1] = true;

	int prev = n-1;
	for(int i=n-2;i>=0;i--)
	{
		if(sgn(v[n].cross(v[prev],v[i])) <= 0) 
		{
			seen[i] = true;
			prev = i;
		}
	...
	for(int i=n-2;i>=0;i--)
	{
		...
		if(prev == i+1)
		{
			ans += v[i].distance(v[prev]);
		}
		else
		{
			Point<ld> temp = line_line_intersection(Line<ld>(v[i],v[i+1]),Line<ld>(v[prev],v[n]));
			ans += v[i].distance(temp);
		}
		prev = i;
	}
