...
void bfs2(int n1,int m,node* orig,vector<vector<int> > &v)
{
  queue<node*> q;
  q.push(orig);
  ...
  while(!q.empty())
  {
    ...
    if(n->level==0 && n!=orig && keep.find(n)==keep.end()) keep.insert(n);
  }
}
...
void bfs(int n1,int m,vector<vector<int> > &v,vector<int> &speed)
{
  ...
  for(int i=1;i<=9;i++) people.push(i);
  for(int i=0;i<n1;i++)
  {
    for(int j=0;j<m;j++)
    {
