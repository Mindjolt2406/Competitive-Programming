...
template <typename T> class Matrix
{
  public:
    ...
    Matrix(int row,int col,const T& initial = 0, ll mod = 1e18+1)
    {
      this->row = row;
      this->col = col;
      this->mod = mod;
    ...
    Matrix<T> operator*(const Matrix<T> &rhs)
    {
      ...
      for(int i=0;i<row;i++)
      {
        for(int j=0;j<rhs.col;j++)
        {
          for(int k=0;k<col;k++)
          {
            res[i][j]+=(1LL*mat[i][k]*rhs(k,j));
            // if(res[i][j]>=mod) res[i][j]-=m;
          }
        }
      }
      return res;
...
int main()
{
  ...
  ld p;
  cin >> n >> p;
  Matrix<ld> exp(2,2),mat(2,1);
  exp[0][0] = exp[1][1] = 1-p;
  exp[0][1] = exp[1][0] = p;
  mat[0][0] = 1;
  mat[1][0] = 0;

  Matrix<ld> final_exp = exp.power(n);
  final_exp *= mat;
  
  cout << fixed << setprecision(12);
  cout << final_exp[0][0] << endl;
  return 0;
}