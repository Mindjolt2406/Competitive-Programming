...
int main()
{
  ...
  int n,m,q;
  cin >> n >> m >> q;
  Matrix<ll> exp(n,n,0,MOD);
  Matrix<ll> orig(n,n,0,MOD);
  ...
  vector<pair<pair<ll,int> ,pair<int,int> > > v(q);
  for(int i=0;i<q;i++)
  {
    cin >> v[i].se.fi >> v[i].se.se >> v[i].fi.fi;
    v[i].se.fi--;
    v[i].se.se--;
    v[i].fi.se = i;
  }
  ...
  for(int i=0;i<q;i++)
  {
    if(prev != v[i].fi.fi)
    {
      exp *= orig.power(v[i].fi.fi-prev,MOD);
      prev = v[i].fi.fi;
    }
    int s = v[i].se.fi, t = v[i].se.se;
    ans.pu(mp(v[i].fi.se,exp[s][t]));
  }

  sort(ans.begin(),ans.end());
  for(auto it : ans) cout << it.se << endl;
  return 0;
}
