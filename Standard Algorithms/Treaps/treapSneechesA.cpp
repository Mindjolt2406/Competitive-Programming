...
bool getProp(Treap* root)
{
  return (root ? root->toProp : 0);
}
...
void propageTreap(Treap* root)
{
  ...
  // t(root->toProp, root->value);
  if(root->left != NULL) root->left->toProp ^= 1;
  if(root->right != NULL) root->right->toProp ^= 1;
...
int main()
{
  ...
  while(q--)
  {
    ...
    if(index == 1)
    {
      // queryLength(root, l, r);
      cout << queryLength(root, l, r) << endl;
      // printInOrderTraversal(root); cout << endl;
    }
  }
  return 0;
