...
{
  int l1 = p1.fi, r1 = p1.se, l2 = p2.fi, r2 = p2.se;
  int s1 = inv[l1], s2 = inv[l2];
  int k = sp.queryMin(min(s1,s2),max(s1,s2)-1);

  int size1 = r1-l1+1, size2 = r2-l2+1;
  if(size1 <= k && size2 <=k)
  {
    if(size1 == size2) return p1 < p2;
    if(size1 < size2) return true;
    else return false;
  }
  else if(size1 <= k && size2 > k) return true;
  else if(size1 > k && size2 <= k) return false;
  else 
  {
