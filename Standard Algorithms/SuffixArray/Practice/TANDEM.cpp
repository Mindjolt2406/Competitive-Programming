...
// void makeLCS(string &s,Sparse &sp_lcs)
// {
//   vector<int> prefix,lcs;
...
int main()
{
  ...
  for(int len = 1;len*3<=orig_n;len++)
  {
    for(int i=0;(i+2)*len < orig_n;i++)
    {
      int x = i*len, y = x + len, z = y+len;
      int prefx = inv_prefix[x], prefy = inv_prefix[y], prefz = inv_prefix[z];
      int suffx = inv_suffix[x], suffy = inv_suffix[y], suffz = inv_suffix[z];

      int prefxy = sp_lcs.queryMin(min(prefx,prefy),max(prefx,prefy)-1);
      int prefxz = sp_lcs.queryMin(min(prefx,prefz),max(prefx,prefz)-1);
      int prefyz = sp_lcs.queryMin(min(prefz,prefy),max(prefz,prefy)-1);

      int suffxy = sp_lcp.queryMin(min(suffx,suffy),max(suffx,suffy)-1);
      int suffxz = sp_lcp.queryMin(min(suffx,suffz),max(suffx,suffz)-1);
      int suffyz = sp_lcp.queryMin(min(suffz,suffy),max(suffz,suffy)-1);

      int leftAns = min(min(prefxy,prefxz),prefyz);
      int rightAns = min(min(suffxy,suffxz),suffyz);

      int totalRange = min(leftAns,len) + min(rightAns,len) - 1;

      if(totalRange >= len)
      {
        boring += totalRange - len + 1;
