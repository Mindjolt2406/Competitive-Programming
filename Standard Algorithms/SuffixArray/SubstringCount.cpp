  ...
  while(q--)
  {
...

    {
      ...
      while(beg <= end)
      {
        ...
        {
          if(s.substr(startIndex,min(s.size()-startIndex,query.size())) == query) ansRight = mid;
          beg = mid+1;
        }
        else 
        {
...

    {
      ...
      while(beg <= end)
      {
        ...
        {
          if(s.substr(startIndex,min(s.size()-startIndex,query.size())) == query) ansLeft = mid;
          end = mid-1;
        }
      }
    }

    if(ansLeft == -1) cout << 0 << endl;
    else cout << ansRight-ansLeft+1 << endl;
  }
  return 0;
}
