...
int main()
{
  ...
  // for(auto it : set_t) cout << it << " "; cout << endl;
  ...
  for(auto it : set_s)
  {
    if(set_t.upper_bound(it) != set_t.end())
    {
      int minIndex = *set_t.upper_bound(it);
      int queryAns = sp.queryMin(it,minIndex-1);
      if(queryAns == max1)
      {
        maxIndex = min(maxIndex,it);
      }
      else if(queryAns > max1)
      {
        max1 = queryAns;
        maxIndex =it;
      }
  ...
  for(auto it : set_t)
  {
    if(set_s.upper_bound(it) != set_s.end())
    {
      int minIndex = *set_s.upper_bound(it);
      int queryAns = sp.queryMin(it,minIndex-1);
      if(queryAns == max1)
      {
        maxIndex = min(maxIndex,it);
  ...
  maxIndex = suffix[maxIndex]; // Convert pos in suffix array to index in original string

  for(int i=maxIndex;i<maxIndex+max1;i++) cout << s[i]; cout << endl;
  return 0;
}
