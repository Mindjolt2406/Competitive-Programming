...
int main()
{
  ...
  cin >> s;
  ll n = s.size();
  vector<int> suffix,lcp,inv;
  Sparse sp;
  suffixArray(s,suffix);
  lcpArray(suffix,s,lcp);
  sp.initMin(lcp);

  vector<int> indexCounter(suffix.size()+1);

  ll cnt = 0;
  int cumSum = 1;
  for(int i=0;i<suffix.size();i++)
  { 
    int startIndex = cumSum + indexCounter[i];
    for(int subSize=startIndex;subSize<=lcp[i];subSize++)
    {
      int beg = i, end = suffix.size()-1, ans = -1;
      while(beg <= end)
      {
        int mid = (beg+end) >> 1;
        if(subSize <= sp.queryMin(i,mid))
        {
          beg = mid+1;
          ans = mid;
        }
      ...
      if(ans != -1)
      {
        ...
        indexCounter[ans+1]--;
        ll sz = ans - i + 1;
        ll temp_cnt = (sz*(sz-1))/2;
        cnt += temp_cnt;
      }
      else break;
    }

    cumSum += indexCounter[i];
  }
