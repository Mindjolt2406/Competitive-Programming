...
const int MAXN = 3e5+1,MAXLOG = 25;
// vector<int> adj[MAXN] = {};
...
int dfs1(ll *sz,vector<int> *adj,int u = 0,int p = -1)
{
  sz[u] = 1;
  for(auto v : adj[u]) if(v-p) sz[u]+=dfs1(sz,adj,v,u);
  return sz[u];
}

void dfs2(ll *sz, vector<int> *adj, vector<vector<int> > &parent, int*height, ll*counter1,vector<vector<ll> > &dp,int u = 0,int p = -1)
{
  // t(u);
  ...
  for(auto v : adj[u]) if(v-p) dfs2(sz,adj,parent, height, counter1,dp,  v,u);
}

int LCA(int*height, vector<vector<int> > &parent, int u,int v)
{
...
ll find(int*height, vector<vector<int> > &parent, vector<vector<ll> > &dp,ll *sz, ll *counter1, int u,int lca,int n,int v = -1)
{
  int u1 = u, v1 = v;
...
int main()
{
  ...
  while(t--)
  {
    ...
    // Localising all the global arrays
    ll sz[MAXN] = {0};
    vector<int> adj[(int)3e5+10] = {};
    int height[(int)3e5+10] = {0};
    // int parent[MAXN][MAXLOG] = {0};
    vector<vector<int> > parent(n,vector<int> (MAXLOG));
    vector<vector<ll> > dp(n,vector<ll> (MAXLOG));
    ll counter1[MAXN] = {0};

    for(int i=0;i<MAXN;i++) for(int j=0;j<MAXLOG;j++) parent[i][j] = -1;
    for(int i=0;i<n-1;i++)
    {
      int a,b;
      cin>>a>>b;
    ...
    dfs1(sz,adj);
    dfs2(sz,adj,parent,height,counter1,dp);
    ...
    while(q--)
    {
      ...
      cin>>a>>b;
      a--;b--;
      int lca = LCA(height,parent,a,b);
      if(lca==a) {a = b; b = lca;}

      if(lca!=a && lca!=b) cout<<find(height,parent, dp,sz,counter1,a,lca,n,b)<<endl;
      else cout<<find(height,parent, dp,sz,counter1,a,b,n)<<endl;
    }
