...
void f(vector<int> &v, vector<int> &w, string &t)
{
  v.clear();
  w.clear();
  v.resize(t.size());
...
int main()
{
  ...
  for(int i=0;i<t.size();i++) 
  {
    if(t[i] == '0') {t[i] = '1'; boo = true;}
    else t[i] = '0'; 
  }
  f(v,w,t);
  if(boo) temp = multiply(v,w);
  for(int i=0;i<ans.size();i++) ans[i] += temp[i];

  // Type 1
  t = s;
  for(int i=0;i<t.size();i++)
  {
    if(t[i] == '1') {t[i] = '1'; boo = true;}
    else t[i] = '0';
  }
  f(v,w,t);
  if(boo) temp = multiply(v,w);
