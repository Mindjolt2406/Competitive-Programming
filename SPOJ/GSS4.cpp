...
node build(node* tree,ll*l,int pos,int beg,int end,int k,int n)
{
  ...
  if(beg<=end)
  {
    ...
    ll value = l[pos-k/2];
    if(beg==end){tree[pos].sum = value;return tree[pos];} //Leaf node
    int mid = (beg+end)/2;
    node n1,n2;
...
node update(node*tree,int index,ll key,int pos,int beg,int end)
{
  ...
  {
    // Here take care of the leaf nodes condition. Also, remember to return tree[pos]
    if(beg==end){tree[pos].sum = key;return tree[pos];} //Leaf node
    int mid = (beg+end)/2;
    node n1,n2;
    if(index<=mid)
    {
...
int main()
{
  ...
  while(getline(cin,s))
  {
    ...
    for(int i=0;i<k;i++)cout<<"i: "<<i<<" sum: "<<tree[i].sum<<endl;
    int q;
    cin>>q;
    for(int i=0;i<q;i++)
    {
