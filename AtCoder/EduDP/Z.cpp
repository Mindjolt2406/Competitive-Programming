...
int main() {
    ...
    // dp[i] = min_j dp[j] + C + (v[i] - v[j])^2
    // Last interval ends at some j. Next interval is cost(j, i) = C + (v[i] -
    // v[j])^2.

    vector<ll> dp(n);
    DynamicLiChaoTree<ll, 1, (int)1e6, 0, false> liTree;
    liTree.init();

    for (int i = 0; i < n; i++) {
        // LiTree considers ending some interval and starting some other one.
        dp[i] = liTree.query(v[i]) + C + v[i] * v[i];
        // Start an interval from i.
        dp[i] = min(dp[i], C + (v[i] - v[0]) * (v[i] - v[0]));

        // Add current line to tree. No point in adding last line.
        if (i != n - 1)
            liTree.add(-2 * v[i], v[i] * v[i] + dp[i]);
    }
