...
// 5. zigzags and zigzigs (For debugging)
...
int height(node* root) // Gets the height of a node
{
  if(root==NULL) {return -1;}
  else {return root->height;}
}

void level(node* root) //Prints the nodes in a level order
{
  if(root != NULL)
  {
    deque< node* > q;
...
node* decide(node* z)
{
  ...
  if(height(z->right) > height(z->left))
  {
    ...
    {
      x = y->right;
      z = zigzig(z,y,x,z->left,y->left,x->left,x->right); //The parameters are z,y,x,t1,t2,t3,t4
    }
    else 
    {
      x = y->left;
      z = zigzag(z,y,x,z->left,x->left,x->right,y->right);
    }
  }
  else
  {
    ...
    {
      x = y->right;
      z = zagzig(z,y,x,y->left,x->left,x->right,z->right);
    }
    else 
    {
      x = y->left;
      z = zagzag(z,y,x,x->left,x->right,y->right,z->right);
    }
  }
  return z;
}
