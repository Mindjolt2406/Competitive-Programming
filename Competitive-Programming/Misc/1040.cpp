...
int dfs(vector < pair <int,int> > adj[],int u,int visited[],int key)
{
  if(u==key) return 1;
  ...
  {
    int v;
    v = dfs(adj,adj[u][i].first,visited,key);
    if(v ==1 && min1<adj[u][i].second) {min1 = adj[u][i].second; boo = 1;}
  }
  if(boo) return 1;
  return 0;
}
