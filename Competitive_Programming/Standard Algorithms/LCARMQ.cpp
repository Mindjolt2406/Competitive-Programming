...
node build(node* tree,int*l,int pos,int beg,int end,int k,int n)
{
  ...
  if(beg<=end)
  {
    ...
    // Eg.{tree[pos].maxsum = l[pos-k/2];tree[pos].maxnumber = l[pos-k/2];return tree[pos];} //Leaf node
...
node update(node*tree,int index,int key,int pos,int beg,int end)
{
  ...
  if(beg<=end)
  {
    if(beg==end){tree[pos].data = key;return tree[pos];} // Leaf nodes
    int mid = (beg+end)/2;
    node n1,n2;
    if(index<=mid)
    {
...
int main()
{
  ...
  for(int i=0;i<q;i++)
  {
    ...
    else if(q1=='Q')
    {
      ...
      cin>>beg>>end;
      beg-=1;end-=1;
      cout<<find(tree,beg,end,0,(k+1)/2 - 1,0,k,(k+1)/2).data<<endl;
    }
  }
  return 0;
}
